...
const DEFAULT_REWARD_AD_COINS = 15;

/** 東京日付（YYYY-MM-DD）ごとのリワード広告コイン付与上限（不正対策・ポリシー用の安全弁） */
...
export async function getRewardAdCoinAmount(): Promise<number> {
  try {
    const rc = admin.remoteConfig();
    const template = await rc.getTemplate();
    const n = parseRcInt(template, RC_KEYS.rewardAdCoins, DEFAULT_REWARD_AD_COINS);
    return Math.max(0, n);
  } catch (e) {
    logger.warn("getRewardAdCoinAmount: Remote Config fallback", e);
    return DEFAULT_REWARD_AD_COINS;
  }
}
